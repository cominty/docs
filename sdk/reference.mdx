...
<Note>
  `export`'s `format` is `"pdf"` or `"docx"`. `upload_file`'s `content` is
  either raw `bytes` or a path (`str`/`pathlib.Path`) to read them from;
  `filename` is limited to 255 characters and can't contain `/`, checked
  locally before any request.
</Note>

<Warning>
  `upload_file` orchestrates 3 HTTP calls, not one: it requests a presigned
  upload permission, uploads your content directly to that storage URL (a
  different host — no Cominty token is sent there), then confirms the upload
  with Cominty. `download_file` similarly follows a presigned URL to a
  different host to fetch the bytes. Both are single SDK calls; the
  multi-step flow only matters if you're calling the raw HTTP endpoints
  yourself — see the [API reference](/api-reference) for the manual steps.
</Warning>
