...
stream its progress, continue the thread, manage threads, and store per-user
memory files an agent can read back later. Everything is scoped to a single
end user (`user_id`) that you set once on the client.
