...
stream its progress, continue the thread, manage threads, cancel or export a
message, upload and download files, and store per-user memory files an agent
can read back later. Everything is scoped to a single end user (`user_id`)
that you set once on the client.
...
<Info>
  **Package** `cominty-sdk` ([![PyPI](https://img.shields.io/pypi/v/cominty-sdk.svg)](https://pypi.org/project/cominty-sdk/)) ·
  **import** `cominty_sdk` · **Python** 3.9+ · **style** async-only
  ([`httpx`](https://www.python-httpx.org/)), fully typed.
</Info>
...
<AccordionGroup>
  ...
  <Accordion title="Resources — chat, threads, and memory" icon="layers">
    `client.chat` starts and continues conversations, streams progress,
    cancels or exports a message, and uploads/downloads files.
    `client.threads` lists, fetches, updates, and archives threads.
    `client.memory` creates, lists, reads, updates, and deletes per-user memory
    files — all scoped to the client's user. See
    [Resources](/sdk/reference#resources).
  </Accordion>
